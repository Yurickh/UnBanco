...
void MainCusMenu::schedulePayment()
{
	...
	bool invalidEnd, invalidDate, invalidPay;
	...
	do
	{
		...
		do
		{
			...
			win->print("Insira a data de pagamento: (DDMMAAAA)");
			win->read(dateInt);

			try
			{
				date = new PayDay(dateInt);
				invalidDate = false;
			} catch (invalid_argument except)
			{
				win->error(except.what());
		...
		do
		{
			...
			win->print("Insira o valor do pagamento a ser realizado");
			win->read(payFlt);
		
			try
			{
				payValue = new Money(payFlt);
				invalidPay = false;
			} catch (invalid_argument except)
			{
				win->error(except.what());
		...
		try
		{
			payCode = transacAdm->schedulePayment(session->getAccNumber(), payValue, date);
			invalidEnd = false;		
		} catch (invalid_argument except)
		{
			win->error(except.what());
	...
	win->success("Pagamento devidamente agendado. Codigo " + payCode->getValue());
	win->pause();
...
void MainCusMenu::cancelPayment()
{
	...
	do
	{
		...
		{
			invalidCode = true;
			win->print("Insira o codigo do pagamento que deseja cancelar");
			win->read(payCodeInt);

			try
			{
				payCode = new PayCode(payCodeInt);
				invalidCode = false;
			} catch(invalid_argument except)
			{
				win->error(except.what());
		...
		try
		{
			transacAdm->deletePayment(payCode);
			invalidEnd = false;
		} catch (invalid_argument except)
		{
			win->error(except.what());
	...
	win->success("Pagamento devidamente cancelado.");
	win->pause();
...
void MainCusMenu::listPayment()
{
	...
	try
	{
		payList = transacAdm->fetchPayment();
		invalidList = false;
	} catch (PersError except)
	{
		win->error(except.what());
	...
	for(it = payList.begin(); it!=payList.end(); ++it)
	{
		win->print("-> Codigo:");
		win->print(it->getPayCode().getValue());
		win->print("-> Data:");
		win->print(it->getPayDay().getValue());
		win->print("-> Valor:");
		win->print(it->getPayValue().getValue());
		win->print("\n");
	}
